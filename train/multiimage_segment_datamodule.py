...
# load csv file
def load_csv(csv_file, topdir, num_slices):
    rets = []
    with open(csv_file) as f:
        ...
        for row in rows:
            ...
            for n in range(num_slices):
                image_path = f"{topdir}/{row[n]}"
                image_paths.append(image_path)
            mask_path = f"{topdir}/{row[num_slices]}"
            clas = int(row[num_slices+1])
            circum = float(row[num_slices+2])
            opt_circum = float(row[num_slices+3])
            
            # return values
            ret = {}
            for n in range(num_slices):
                ret[f'image{n}'] = image_paths[n]
...
class MultiimageSegmentDataModule(pl.LightningDataModule):
    def __init__(self, cfg):
        super(MultiimageSegmentDataModule, self).__init__()
    ...
    def setup(self, stage):
        ...
        train_datalist = load_csv(self.cfg.Data.dataset.train_datalist,
                                  ...
                                  self.cfg.Data.dataset.num_slices)
